...
tStatus arcInXYPlane( double X, double Y, double Z, double I, double J, double P )
{
  ...
  stepCount = l / ( getSmalestStep( ) * 100 );
...
tStatus doGcode( char* cmd )
{
  ...
  {
    if( sscanf( pt+1, "%lf", &val ) != 1 ) return retInvalidParam; 
    M.X = val;
    gotWhat |= GOT_X;
  }
  ...
  {
    if( sscanf( pt+1, "%d", &i ) != 1 ) return retInvalidParam; 
    switch( i )
    {
      case 0 :
      case 1 :        
  ...
  {
    pt++;
    if( sscanf( pt, "%d", &n ) != 1 ) return retInvalidParam;
